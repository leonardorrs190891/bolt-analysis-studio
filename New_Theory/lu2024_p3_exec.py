# -*- coding: utf-8 -*-
"""Executor do prereg P3 LU_2024 (2026-07-31-lu2024-p3-prereg.md).
...
Saida: lu2024_p3_exec.json + prints ASCII.
"""
...
def _ov(emb_um, frac, nemb, kr=None, floor=None):
    ov = {"c_bend": C_BEND, "emb_slip_gate": Q_GATE,
...
def main() -> int:
    ...
    A = []
    for emb in (0.0, 6.0, 11.0):
        for frac in (0.5, 0.6, 0.7):
            for nemb in (0.5, 1.0, 2.0):
                ov = _ov(emb, frac, nemb)
                ...
                A.append({"emb": emb, "frac": frac, "nemb": nemb,
                          "mae_c1": mae, "mono": mono, "det": det})
                print(f"  emb={emb:4.1f} frac={frac:.1f} N={nemb:3.1f} "
                      f"MAE(c1)={mae:.4f} mono={'S' if mono else 'n'}")
    out["estagioA"] = [{k: v for k, v in a.items() if k != "det"} for a in A]
    surv = sorted([a for a in A if a["mono"]], key=lambda a: a["mae_c1"])[:4]
    if not surv:
    ...
    melhor = None
    for s in surv:
        for kr in (0.005, 0.01, 0.02, 0.04):
            for fl in (0.0, 0.10, 0.21):
                ov = _ov(s["emb"], s["frac"], s["nemb"], kr, fl)
                maes, sds = {}, {}
                for cid in READS:
                    r = _sim(cid, ov)
                    maes[cid] = r.mae
                ...
                row = {**{k: s[k] for k in ("emb", "frac", "nemb")},
    ...
    if passa and "--adotar" in sys.argv:
        ...
        prov = ("P3 re-leitura sob drive corrigido (fig20 1.0mm): c_bend "
                ...
                "prereg 2026-07-31-lu2024-p3; adotado por delegacao "
    ...
    (ROOT / "New_Theory" / "lu2024_p3_exec.json").write_text(
