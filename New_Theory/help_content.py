...
PASSOS = [
    dict(chave="wizard", print_="chrome_model", pt=(
        ...
        "perguntas, use <b>Arquivo &rarr; Importar caso da validacao</b> "
        "(<kbd>Ctrl</kbd>+<kbd>I</kbd>): o dialogo lista por nome as 205 "
        "curvas do censo do artigo, com a fonte, o DOI e o erro do modelo em "
        "cada uma, e o caso escolhido chega montado e com as constantes "
        "adotadas daquele artigo. O mesmo caso tambem pode ser aberto pelo "
        "modulo <b>Results &rarr; Validation</b>, em <b>Abrir no "
        "Model/Run</b>."), en=(
        ...
        "use <b>File &rarr; Import validation case</b> "
        "(<kbd>Ctrl</kbd>+<kbd>I</kbd>): the dialogue lists by name the 205 "
        "curves in the paper census, with the source, the DOI and the model "
        "error on each, and the chosen case arrives assembled and with the "
        "adopted constants of that paper. The same case can also be opened "
        "from <b>Results &rarr; Validation</b>, via <b>Abrir no "
        "Model/Run</b>.")),
...
DIALOGOS = {
    ...
    "Importar caso da validação": dict(pt=(
        ...
        en=("Confirms the paper case loaded in its adopted configuration.",
        "Shown after File > Import validation case (Ctrl+I). The dialogue "
        "lists by name the 205 curves in the paper census; clear 'census "
        "only' to see all 210 in the repository. The source file is version "
        "controlled and regenerated by the case builder, so the next Ctrl+S "
        "asks for a new destination instead of overwriting the reference.")),
