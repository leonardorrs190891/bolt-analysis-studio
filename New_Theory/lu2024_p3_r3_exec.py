# -*- coding: utf-8 -*-
"""Executor do prereg P3-R3 LU_2024 (2026-07-31-lu2024-p3-r3-prereg.md).

Baseline P5+P2: T4 DECLARADA (informacional, fora do G4); amp1p0 duplicata
(fora de tudo, reportada). c1 como RESTRICAO do estagio B (mae_c1<=0.08).
...
Saida: lu2024_p3_r3_exec.json + prints ASCII.
"""
...
READS = ["lu2024_M8_fig18_amp0p25",
         "lu2024_M8_fig20_T22Nm", "lu2024_M8_fig20_T10Nm",
         "lu2024_M8_fig18_amp2p0"]
DUP = "lu2024_M8_fig18_amp1p0"   # P2: duplicata da T22 — so informacional
...
def main() -> int:
    ...
    surv = sorted([a for a in A if a["mono"] and a["ok025"] and a["okc1"]],
    ...
    for cid in READS + HELD + [T4, DUP]:
        r = _sim(cid, ov)
        dep_all[cid] = r
        if cid in (T4, DUP):          # P5/P2: informacionais, fora do G4
            b = base.get(cid)
            if b is not None:
                print(f"  (info) {cid.split('_')[-1]}: mae "
                      f"{b.mae:.4f}->{r.mae:.4f} sd "
                      f"{b.resid_std:.4f}->{r.resid_std:.4f}")
            continue
        b = base[cid]
    ...
    # G5: rendimento
    cens9 = READS + HELD + [T4]
    med_antes = float(np.median([base[c].mae for c in cens9]))
    med_dep = float(np.median([dep_all[c].mae for c in cens9]))
    ...
    if passa and "--adotar" in sys.argv:
        ...
        prov = ("P3 re-leitura sob drive corrigido (fig20 1.0mm): c_bend "
                ...
                "preregs 2026-07-31-lu2024-p3{,-r2,-r3}; adotado por delegacao "
    ...
    (ROOT / "New_Theory" / "lu2024_p3_r3_exec.json").write_text(
