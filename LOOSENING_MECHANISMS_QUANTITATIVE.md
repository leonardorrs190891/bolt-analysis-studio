...
### 2.2 Embedding quantitative values (roughness → f_Z)

> **Provenance correction (2026-07-07):** the table below is an **engineering-consensus
> guide** (per-interface, by finish), **NOT** VDI 2230-1 Table 5.4/1 (it was mislabeled).
> The *authoritative* VDI Table 5.4/1 has only **three** Rz classes (Rz<10 / 10–40 / 40–160),
> differentiated by location (thread/head/interface) and loading, with **no sub-Rz<10
> class** — its floor (Rz<10) over-predicts fine-ground settling ~4× (see
> `MODEL_LEGITIMACY.md` §4.6). The finer values here are consistent with Bolt Science's
> authoritative range (**1–7 µm/interface, smoother → less**;
> [boltscience.com/pages/embedding.htm](https://www.boltscience.com/pages/embedding.htm),
> externally verified 2026-07-07). The model uses the real 3-class VDI table
> (`New_Theory/library_common.emb_depth_vdi`) **plus** a Bolt-Science-anchored `Rz<4`
> fine-ground class (~1 µm/interface).

Embedding per interface — **engineering-consensus guide** (by surface finish, NOT VDI):
